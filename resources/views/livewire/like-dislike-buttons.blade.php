<div class="flex">
    ...
    <div class="flex items-center mt-1">
        <button wire:click="dislike">
            <svg viewBox="0 0 20 20" class="{{ $tweet->isDislikedBy(current_user()) ? 'text-blue-500' : 'text-gray-500' }} mr-1 w-4">
                <g id="Page-1" stroke="none" stroke-width="1" fill="none" fill-rule="evenodd">
                    <g class="fill-current">
        ...
        <span class="text-xs {{ $tweet->isDislikedBy(current_user()) ? "text-blue-500" : "text-gray-500" }}">
            {{ $tweet->count_of_dislikes->count() ?: 0 }}
        </span>
    </div>
